...
const getSingle = async (req, res) => {
    const techId = new ObjectId(req.params.id);
    const result = await mongodb.getDatabase().collection('tech').find({ _id: techId });
    result.toArray()
        .then((tech) => {
            res.setHeader('Content-Type', 'application/json');
...
const postTech = async (req, res) => {;
    ...
    const response = await mongodb.getDatabase().collection('tech').insertOne(newTech);
...
const putTech = async (req, res) => {
    ...
    const response = await mongodb.getDatabase().collection('tech').replaceOne({ _id: techId }, updatedTech);
...
const deleteTech = async (req, res) => {
    const techId = new ObjectId(req.params.id); 
    const response = await mongodb.getDatabase().collection('tech').deleteOne({ _id: techId });
